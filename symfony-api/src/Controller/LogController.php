...
class LogController extends AbstractController
{
    ...
    public function update(LogEntry $entry, Request $request, EntityManagerInterface $em): JsonResponse
    {
        $this->hydrate($entry, $this->payload($request));
        $em->flush();
    ...
    {
        if (isset($data['title'])) {
            $entry->setTitle((string) $data['title']);
        }
        if (array_key_exists('year', $data)) {
            $entry->setYear($data['year'] !== null ? (int) $data['year'] : null);
        }
        if (array_key_exists('genre', $data)) {
            $entry->setGenre($data['genre']);
        }
        if (isset($data['rating'])) {
            $entry->setRating((float) $data['rating']);
        }
        if (array_key_exists('review', $data)) {
            $entry->setReview($data['review']);
        }
        if (!empty($data['watchedAt'])) {
            $entry->setWatchedAt(new \DateTimeImmutable($data['watchedAt']));
        }
    }
}
